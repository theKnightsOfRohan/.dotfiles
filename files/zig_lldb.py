...
def create_struct(parent, name, struct_type, inits):
    ...
    for field in struct_type.fields:
        ...
        if isinstance(field_init, int):
            if struct_data.byte_order == lldb.eByteOrderLittle:
                    byte_order = 'little'
            elif struct_data.byte_order == lldb.eByteOrderBig:
                    byte_order = 'big'
...
class root_InternPool_Index_Unwrapped_SynthProvider:
    ...
    def update(self):
        ...
        if encoding_index:
            ...
            if index_data.byte_order == lldb.eByteOrderLittle:
                    index_bytes = bytes(index_bytes[:index_type.size])
            elif index_data.byte_order == lldb.eByteOrderBig:
                    index_bytes = bytes(index_bytes[-index_type.size:])
            ...
            if data_data.byte_order == lldb.eByteOrderLittle:
                    data_bytes = bytes(data_bytes[:data_type.size])
            elif data_data.byte_order == lldb.eByteOrderBig:
                    data_bytes = bytes(data_bytes[-data_type.size:])
            ...
            if encoding_trailing and encoding_config:
                ...
                for trailing_field in trailing_type.fields:
                    ...
                    while True:
                        for trailing_field_type_field in trailing_field_type.fields:
                            if trailing_field_type_field.name == 'has_value' or trailing_field_type_field.name == '?':
                                # case ['has_value', '?']:
                                    has_value_field, child_field = trailing_field_type.fields
                                    trailing_field_name = '%s.%s' % (trailing_field_name, child_field.name)
                                    if eval_config(trailing_field_name).value == 'true':
                                        ...
                                        # case 'false':
                                            ...
                                            break
                            elif trailing_field_type_field.name == 'ptr' or trailing_field_type_field.name == 'len':
                                ...
                                if trailing_data.byte_order == lldb.eByteOrderLittle:
                                ...
                                elif trailing_data.byte_order == lldb.eByteOrderBig:
                                        len_field_bytes = len_field_value.data.uint8[-len_field_size:]
